...
//[GFN]--------------------------------
int _gfnDebugMode = 0;
float _gfnObjectCoordsTranslateX = 0.0;
float _gfnObjectCoordsTranslateY = -0.54;
float _gfnObjectCoordsTranslateZ = 0.0;
float _gfnObjectCoordsScaleX = 8.0;
float _gfnObjectCoordsScaleY = 8.0;
float _gfnObjectCoordsScaleZ = 8.0;
...
// GFN MToon Variation
    ...
    objectCoords = float4(-objectCoords.x, -objectCoords.z, objectCoords.y, objectCoords.w);
    worldNormal = half3(-worldNormal.x, -worldNormal.z, worldNormal.y);

    if(_gfnDebugMode == 1)
        return float4(objectCoords.xyz, 1.0);
    if(_gfnDebugMode == 3)
        return float4(
    		(abs(objectCoords.x) > 1.0 ? 1.0 : (objectCoords.x >= 0.0 ? 0.5 : 0.0)),
    		(abs(objectCoords.y) > 1.0 ? 1.0 : (objectCoords.y >= 0.0 ? 0.5 : 0.0)),
    		(abs(objectCoords.z) > 1.0 ? 1.0 : (objectCoords.z >= 0.0 ? 0.5 : 0.0)),
    		1.0
    	);
    ...
    if(_gfnDebugMode == 2)
        return float4(worldNormal.xyz, 1.0);

    worldNormal = normalize(worldNormal);

     // Convert to unity coords
    worldNormal = half3(-worldNormal.x, worldNormal.z, -worldNormal.y);
    worldNormal = mul(unity_ObjectToWorld, float4(worldNormal, 0.0)).xyz;
